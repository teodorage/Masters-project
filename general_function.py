...
def hyperbolic_trajectory_calculator(r,eccentricity,semimajor_axis, arg_periapsis):
    ...
    times =  np.arange(0,2*np.pi/mean_motion,1000)
...
def elliptical_trajectory_calculator(r,eccentricity, semimajor_axis, arg_periapsis):
    ...
    mean_motion = np.sqrt(np.divide(constants.MU_JUPITER,(semimajor_axis**3)))
    t_periapsis = M_start/mean_motion
    times = np.arange(0,0.7*np.pi/mean_motion, 1000)
    # print(E_start,M_start,mean_motion,2*np.pi/mean_motion, semimajor_axis/constants.R_JUPITER, np.linalg.norm(eccentricity))
    meananomaly = -mean_motion*(times) + M_start
    ...
    trueanomaly = 2.0*np.arctan2((np.tan(eccentricanomaly/2)),1/(np.sqrt((np.linalg.norm(eccentricity)+1)/(1-np.linalg.norm(eccentricity)))))

    if eccentricity[1]<0.0:
        arg_periapsis = -arg_periapsis
    # if np.linalg.norm(eccentricity)>0.9:
    #     trueanomaly= -trueanomaly

    radial = semimajor_axis*(1-np.multiply(np.linalg.norm(eccentricity),np.cos(eccentricanomaly))) # determines the radius which is then used for the x-y coordinates
    gamma  = np.arctan2((np.linalg.norm(eccentricity)*np.sin(trueanomaly)),(1+np.linalg.norm(eccentricity)*np.cos(trueanomaly))) #flight path angle         
...
def determine_orbit(r0,v0,time_start,arc_max,arc_num,results):
    ...
    if arc_num<arc_max:
        ...
        print(np.linalg.norm(eccentricity))
        ...
        results[key] = {"starting position [km]":r0, "spacecraft_positions":position, "starting velocity [km\s]":v0,"semimajor axis": semimajor_axis, 
        ...
        if ca_flyby == 1:
            ...
            arc_num += 1
            print(new_position,v_inf,arc_num,np.degrees(delta))
...
r = [46192695.552634776, 54565015.72969255, 0] 
v= [-2.41463027711917, -2.3936500631502926, 0]
...
# def grid_search(min_deg_r=49.0, max_deg_r=51.0, delta_deg_r=0.15,
#                 min_deg_v=175.0, max_deg_v=176.0, delta_deg_v=0.2):
...
# grid_search()
plt.gca().add_artist(matplotlib.patches.Circle((0,0), constants.R_JUPITER, facecolor="none", edgecolor="r"))
plt.show()
